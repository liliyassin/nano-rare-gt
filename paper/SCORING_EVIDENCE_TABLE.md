...
|---|---:|---|---|---|---|
| Packaging, tropism, protein class, pathway, immunogenicity, therapeutic window | 2.0 each | Higher-weight determinants | Evidence-informed heuristic | These are major feasibility constraints: the gene must fit, vector must reach tissue, biology must be transferable, immune barriers matter, and timing affects trial feasibility. | The choice to give exactly double weight is not statistically derived. |
...
| Raw total | 18.0 | Sum of all dimension maxima | Heuristic | Convenient normalisation denominator. | Not a trained model; weights were not fitted to outcome data. |
...
4. Run a sensitivity analysis with alternate weights, for example equal weights
   for all dimensions and a stricter route/tropism weighting.
5. Update DMD text in `paper/results.md` and `paper/discussion.md` to reflect
   the new microdystrophin strategy output: native full-length DMD fails, but
   SRP-9001/DMD_micro is now surfaced as an engineered-cargo precedent.
