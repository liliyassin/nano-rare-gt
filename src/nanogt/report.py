...
def generate_report(result: MatchResult) -> str:
    ...
    for i, s in enumerate(top, 1):
        lines += [
            ...
            f"| Organelle targeting | {s.organelle_targeting:.2f} | 1.0 | Nuclear AAV delivery reaches correct subcellular compartment |",
            f"| **TOTAL (normalised)** | **{s.composite_score:.2f}** | **10.0** | Raw sum / 21 × 10 (v2: 14 dimensions) |",
            f"",
            f"### Rationale",
            f"",
        ]
