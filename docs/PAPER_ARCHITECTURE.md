...
NanoGT is a proof-of-concept framework for computational gene-therapy precedent mapping. In a 46-disease monogenic rare-disease cohort, it generated interpretable disease-precedent clusters, recovered several expected clinical precedents, and correctly flagged NF1 as a packaging hard-fail (8,451 bp CDS exceeds all catalogued vectors).
...
|---|---:|---|
| Abstract | 250–300 | Match the 46-disease results |
...
| Results | 1,800–2,300 | 46-disease table, clusters, validation, stress tests |
...
1. 46-disease score distribution.
...
1. 46-disease cohort result table.
...
- Every disease fact in `data/disease_cohort_46.csv` must be source-checked.
- Reference section must not contain placeholders.
- Results must use the 46-disease output, not an earlier pilot output.
