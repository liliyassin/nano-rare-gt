...
A static database of 21 approved or late-stage gene therapy programs was manually curated from published literature, regulatory approval documents, and ClinicalTrials.gov. Programs were included if they targeted a monogenic, loss-of-function rare disease using an AAV vector or an integrating ex vivo haematopoietic stem-cell vector. Each entry recorded: program name, target disease, causal gene symbol, vector serotype or vector-class bucket, primary tissue target, transgene CDS length in base pairs, approval or trial stage, approval year (where applicable), protein class (secreted, lysosomal, membrane, intracellular, or secreted_lysosomal), inheritance pattern (AR or XL), and biological pathway label.

Eight delivery vectors or vector-class buckets were catalogued: AAV1, AAV2, AAV5, AAV8, AAV9, AAVrh10, AAV2/6, and an integrating ex vivo HSC vector bucket labelled LV in the scoring outputs. For each vector, the following properties were recorded: cargo limit in base pairs, tissue tropism, CNS/retinal/hepatic/muscle tropism flags, number of existing clinical programs using that serotype or class, and patent restriction status. The standard cargo limit for all AAV serotypes was set at 4,700 bp, consistent with the established packaging limit for single-stranded AAV. The integrating HSC-vector cargo limit was set at 8,000 bp, reflecting the larger capacity of lentiviral and gammaretroviral constructs used in approved haematopoietic stem-cell programs; Strimvelis is a gammaretroviral ADA-SCID product grouped in this bucket for scoring, not a lentiviral product.
