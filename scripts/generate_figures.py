...
ax.set_title("Figure 1: NanoGT Composite Scores — 46-Disease Cohort", fontsize=12, fontweight="bold", pad=12)
ax.set_xlim(0, 11)
...
cases = {
    "Hemophilia B\n(positive control)":       "ORPHA:98879",
    "SMA\n(positive control)":                "ORPHA:70",
    "Hemophilia A\n(positive control)":       "ORPHA:98878",
...
for ax, (title, orpha) in zip(axes, cases.items()):
    ...
    bar_height = 0.5
    ax.barh(y_pos, scores, color=bar_cols, edgecolor="white", linewidth=0.4, height=bar_height)
    ax.set_yticks(y_pos)
    ax.set_yticklabels(programmes, fontsize=7)
    ax.set_xlim(0, 10.5)
    # Always span at least 5 slots so a single-result panel (e.g. DMD) doesn't fill the whole height
    ax.set_ylim(max(len(programmes) - 0.5, 4.5), -0.5)
